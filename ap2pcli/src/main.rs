...
fn main() -> Result<(), isize> {
    ...
    match command.as_str() {
        "conn" | "conns" | "connection" | "connections" => match next_arg()?.as_str() {
            "l" | "-l" | "list" | "--list" => {
        ...
        "msg" | "msgs" | "message" | "messages" => match next_arg()?.as_str() {
            "l" | "-l" | "list" | "--list" => {
        ...
        "help" | "-h" | "--help" => {
            ...
            print!("{}", " ".repeat(50 + prog_path.len()));
            println!("[s | -s | select  | --select ] <ID>");
            print!("{}", " ".repeat(50 + prog_path.len()));
            println!("[r | -r | request | --request] <ADDR>");
            print!("{}", " ".repeat(50 + prog_path.len()));
            println!("[d | -d | decide  | --decide ] <ID> <DECISION>");
            println!();
            println!("       {prog_path} [msg  | msgs  | message    | messages   ] [l | -l | list  | --list ]");
            print!("{}", " ".repeat(50 + prog_path.len()));
            println!("[s | -s | send  | --send ] <MSG>");
            print!("{}", " ".repeat(50 + prog_path.len()));
            println!("[b | -b | bulk  | --bulk ] <MSGS>");
            println!();
